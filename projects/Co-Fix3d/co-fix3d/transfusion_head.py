...
class TransFusionHead(nn.Module):
    ...
    def create_2D_grid(self, x_size, y_size):
        ...
        batch_x = batch_x + 0.5
        batch_y = batch_y + 0.5
        coord_base = torch.cat([batch_x[None], batch_y[None]], dim=0)[None]  # 2 W，H
    ...
    def generate_gt_groups(self, query_feat, query_pos, query_heatmap_score, lidar_feat, lidar_feat_flatten, bev_pos,
                           heatmap,
        ...
        batch_valid_gt_mask = torch.zeros((batch_size, self.max_num_gts * self.add_gt_groups), dtype=bool,
                                          device=query_pos.device)
        for batch_idx in range(batch_size):
            for group_i in range(self.add_gt_groups):
                batch_valid_gt_mask[batch_idx,
                group_i * self.max_num_gts:group_i * self.max_num_gts + self.num_gts[batch_idx]] = 1
        ...
        for batch_idx in range(batch_size):
            ...
            gs_labels = F.pad(gt_labels_3d[batch_idx], pad=(0, self.max_num_gts - self.num_gts[batch_idx]),
                              value=self.num_classes)  # set background class
            gs_bev_corners = F.pad(gt_bboxes_3d[batch_idx].corners.cuda().reshape(-1, 4, 2, 3)[:, :4, 0, :2],
                                   pad=(0, 0, 0, 0, 0, self.max_num_gts - self.num_gts[batch_idx]))
            ...
            if self.add_gt_groups_noise.startswith('rect'):
                ...
                gs_bev_corners_rect = torch.cat(
                    [gs_bev_corners_trans.min(dim=1)[0], gs_bev_corners_trans.max(dim=1)[0]], dim=1)
        ...
        batch_gt_bev_pos = bev_pos.gather(
            index=(batch_gt_pos[:, :, 1] * H + batch_gt_pos[:, :, 0])[..., None].expand(-1, -1, bev_pos.shape[-1]),
            dim=1)
        batch_gt_heatmap_score = heatmap.gather(
            index=(batch_gt_pos[:, :, 1] * H + batch_gt_pos[:, :, 0])[:, None, :].expand(-1, self.num_classes, -1),
            dim=-1)
        batch_gt_query_feat = lidar_feat_flatten.gather(
            index=(batch_gt_pos[:, None, :, 1] * H + batch_gt_pos[:, None, :, 0]).expand(-1,
                                                                                         lidar_feat_flatten.shape[1],
                                                                                         -1), dim=-1)
        ...
        query_heatmap_score = torch.cat(
            [query_heatmap_score, batch_gt_heatmap_score * batch_valid_gt_mask[:, None, :].float()], dim=2)
        ...
        if dense_heatmap_boxes is not None:
            if self.add_gt_groups_noise_box == 'pred':
                ...
                batch_gt_heatmap_box = dense_heatmap_boxes.gather(index=
                                                                  (batch_gt_query_labels.clip(
                                                                      max=self.num_classes - 1) * H * W + batch_gt_pos[
                                                                                                          :, :,
                                                                                                          1] * H + batch_gt_pos[
                                                                                                                   :, :,
                                                                                                                   0]
                                                                   )[:, None].expand(-1, dense_heatmap_boxes.shape[1],
                                                                                     -1), dim=-1)
            elif self.add_gt_groups_noise_box == 'gt':
                batch_gt_heatmap_box = batch_gt_bboxes_3d.transpose(1, 2)
                # x y shift
                batch_gt_heatmap_box[:, :2, :] = (batch_gt_bev_pos * batch_valid_gt_mask[..., None].float()).transpose(
                    1, 2)
                ...
                batch_gt_heatmap_box[:, :2, :] = (batch_gt_bev_pos * batch_valid_gt_mask[..., None].float()).transpose(
                    1, 2)  # x, y
                # length along x y z
                batch_gt_heatmap_box[:, [2], :] = batch_gt_heatmap_box[:, [2], :] + meta_noise[:,
                                                                                    [0]] * batch_gt_heatmap_box[:,
                                                                                           [5]].exp()  # z
                batch_gt_heatmap_box[:, [3, 4, 5], :] = torch.log(
                    batch_gt_heatmap_box[:, [3, 4, 5], :].exp() * (1 + meta_noise[:, [1, 2, 3]]).clip(min=0.1,
                                                                                                      max=3.) + 1e-6)  # l' w' h'
    ...
    def forward_single(self, inputs, metas):
        ...
        fusion_feat = self.diffusion(fusion_feat)
        #################################
        # image to BEV
        #################################
