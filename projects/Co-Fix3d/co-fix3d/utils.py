...
class HungarianAssigner3D(BaseAssigner):
    ...
    def assign(self, bboxes, gt_bboxes, gt_labels, cls_pred, train_cfg, sec_match=False):
        ...
        if sec_match:
            cost_t = cost.clone()
            cost_t[matched_row_inds, :] = 100
            sec_matched_row_inds, sec_matched_col_inds = linear_sum_assignment(cost_t)
            sec_matched_row_inds = torch.from_numpy(sec_matched_row_inds).to(bboxes.device)
            sec_matched_col_inds = torch.from_numpy(sec_matched_col_inds).to(bboxes.device)
            sec_matchs.append(sec_matched_row_inds)
            sec_matchs.append(sec_matched_col_inds)
        return AssignResult(
            num_gts, assigned_gt_inds, max_overlaps, labels=assigned_labels), sec_matchs
