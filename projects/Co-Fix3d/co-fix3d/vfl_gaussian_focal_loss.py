...
def vfl_gaussian_focal_loss(pred: Tensor,
    ...
    # loss = weight_reduce_loss(pos_weight * pos_loss + neg_weight * neg_loss, weight, reduction, avg_factor)
    eps = 1e-12
    pos_weights = gaussian_target.eq(1) * target_score
    # pos_weights = gaussian_target.eq(1) * (
    #         target_score * (target_score > iou_threshold) + (1.5-target_score)*(target_score <= iou_threshold))
    # neg_weights = (1 - target_score).pow(gamma) * ((gaussian_target < 1).float()) * ((target_score > 0).float()) + (1 - gaussian_target).pow(gamma) * (target_score.eq(0))
    neg_weights = (1 - target_score).pow(gamma) * (1 - gaussian_target.eq(1).float())
    pos_loss = -(pred + eps).log() * (1 - pred).pow(alpha) * pos_weights
    neg_loss = -(1 - pred + eps).log() * pred.pow(alpha) * neg_weights
    loss = pos_weight * pos_loss + neg_weight * neg_loss
    loss = weight_reduce_loss(loss, weight, reduction, avg_factor)
    #
    # weight = pred.pow(alpha) * ((gaussian_target == 0).float()) + (gaussian_target > 0).float() * (1 - gaussian_target).pow(gamma) + target_score * gaussian_target.eq(1)
    # loss = F.binary_cross_entropy_with_logits(pred, target_score, weight=weight.detach(), reduction='none')
    # loss = weight_reduce_loss(loss, weight, reduction, avg_factor)
    ...
    # eps = 1e-12
    # pos_weights = gaussian_target.eq(1)*target_score
    # neg_weights = (1 - target_score).pow(gamma)
    # pos_loss = -(pred + eps).log() * (1 - pred).pow(alpha) * pos_weights
    # neg_loss = -(1 - pred + eps).log() * pred.pow(alpha) * neg_weights
    # loss = pos_weight * pos_loss + neg_weight * neg_loss
    # loss = weight_reduce_loss(pos_loss, weight, reduction, avg_factor)
...
class VFLGaussianLoss(nn.Module):
    ...
    def forward(self,
        ...
        if pos_inds is not None:
            ...
            loss_reg = self.loss_weight * vfl_gaussian_focal_loss(
                pred,
                target,
                target_score,
                weight=weight,
